...
export class ProfileComponent implements OnInit {
 ...
 this.authService.getProfile().subscribe({  next: (user: any) => {
  ...
  enableEdit(): void {
    this.isEditMode = true; this.originalUsername = this.username;
    this.originalEmail = this.email; this.originalImage = this.profileImage;
  }
  ...
  onFileSelected(event: any): void {
    ...
    this.authService.uploadImage(file).subscribe({  next: (res: any) => {
      ...
      error: () => { this.profileImage = this.originalImage;
        this.cdr.detectChanges();
      }});
  }
  saveProfile(): void {
    ...
    this.authService.updateProfile({
      username: this.username, email: this.email, currentPassword: this.currentPassword,
      newPassword: this.newPassword, profileImage: this.uploadedImagePath
    })
    .subscribe({ next: () => {
        this.originalUsername = this.username; this.originalEmail = this.email;  this.originalImage = this.profileImage;
        this.currentPassword = ''; this.newPassword = '';
        this.isEditMode = false; this.saving = false;
        this.showCurrentPwd = false; this.showNewPwd = false;
