...
export class SignupComponent {
  ...
  register(): void {
    ...
    this.authService.register({
      username: this.username.trim(), email: this.email.trim(), passwordHash: this.password
    })
    ...
    .subscribe({ next: () => {
      ...
      error: (err) => {
        if (err instanceof TimeoutError) {
          ...
          this.signupError =
            err.error?.message ??
            'User already exists. Please use another email or username.';
        }
        else if (err.status === 400) {
          this.signupError =
            err.error?.message ?? 'Invalid registration details.';
        }
